...
| `N10_EQUAL_ACCOUNTING_CAMPAIGN` | `DONE` | CovQ scored on emitted-readout CFI, QUEST and sparse-Carathéodory on noisy QFI as **optimistic upper bounds**, two-qubit noise charged per emitted gate on every arm. At zero noise **QUEST wins on shots** (1.626 vs 1.766); from edge depolarization `0.01` CovQ wins and the gap runs to five orders of magnitude. CovQ beats product-only everywhere, so the compiler does not degenerate to `F = I`. Sparse-Carathéodory is included but its column reflects **this prototype's** 232-CX dense preparation, an upper bound, not the method. |
