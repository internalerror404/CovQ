...
| `QUEST_BASELINE` | `RUN_EXACT_MODE_ONLY` | arXiv:2605.02367, reimplemented from the published method description. `docs/audits/K3_QUEST_BASELINE_AND_ADAPTIVE_READOUT_v0.1.md`, `results/baselines_k3_quest.json`. **QUEST-tE as published** (insert, then joint L-BFGS reoptimisation). `docs/audits/K3_QUEST_BASELINE_v0.2.md`. **11/11 converge**, ratio range **2.50–32.65** in *emitted CX* (was 2.5–197.8 against the flawed baseline). `bE` agrees with `tE` exactly where both ran — 40 CX total across 8 instances — but coverage is **8/11**, and the three uncovered are `path_m4`, `path_m5`, `banded_m5`: **exactly the three largest ratios**. A strengthening study (`prototype/scripts/quest_strengthening_study.py`) **falsified** the registered arm on the one deep instance it could settle: windowed best-position search found 20 CX against tE's 24 on `path_m4`, so `17.78 → 14.81`. The upper end is therefore an **upper bound on the separation, not a measured one**, and the abstract must say "up to". Two by-products: greedy insertion is **not monotone in its candidate set** (`W = 2` is worse than `W = 1` on 2/2 instances), so no windowed run may be quoted as a bound on `bE`; and random restarts are ~4× worse than `|+⟩^m`, which justifies the registered initialisation. `W = 4` on `path_m5` did not converge within a 30-minute budget and `banded_m5` was not reached — recorded as no verdict, not as confirmation. CovQ 2q depth is 1 everywhere, by the matching theorem, independent of the baseline. |
...
| `N10_EQUAL_ACCOUNTING_CAMPAIGN` | `DONE / WEAKENED` | Rerun against QUEST-tE under **symmetric per-emitted-CX accounting**. QUEST's preparation is 3 two-qubit Pauli rotations = **6 emitted CX** (was reported as 44 gates). Crossover `q_edge ∈ (0.01, 0.02]`; margin at 20 % is **3.20×**, not 1.27e5×. The low-noise rows are **not** operational QUEST wins — QUEST has no compiled readout, so only `F_C,CovQ > F_Q,QUEST` is a real result. Deployable exposure is now computed on **two independent channels** — exact binomial enumeration (no sampling) and Monte-Carlo pilot draws — agreeing to `6.0e-5` relative at worst (`1.1e-5` at the crossing itself), with the exact channel's angle grid converged to `1.5e-6`. Seed spread alone measured only sampler noise; enumeration also bounds bias in the expectation. The narrowest crossing margin, `3.1 %` at `q_edge = 0.02`, clears the worst-case uncertainty by **512×**. Sparse-Carathéodory is included but its column reflects **this prototype's** 232-CX dense preparation, an upper bound, not the method. |
| `QUEST_EMITTED_GATE_ACCOUNTING` | `FIXED` | Three unit errors, one favouring CovQ and two favouring QUEST: rotations reported as gates (a two-qubit rotation is 2 CX); `quest_circuit` omitted its own `|+⟩^m` preparation so the artifact was not self-contained; and noise was charged per rotation rather than per emitted CX, under-billing QUEST 2×. Both arms now share one lowering and noise path. |
